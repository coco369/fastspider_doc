...
命令行工具为**fastspider**内置支持的，可方便快速的创建项目、爬虫、item、以及调试请求等，使用方法如下：
...
打开命令行窗口，输入fastspider
    ...
    Available commands:
      create        create project、fastspider、item and so on
      shell         debug response

    Use "fastspider <command> -h" to see more info about a command


可见fastspider支持`create`及`shell`两种命令

## 2. fastspider create

使用fastspider create 可快速创建项目、爬虫、item等，具体支持的命令可输入`fastspider create -h` 查看使用帮助

    > fastspider create -h
    usage: fastspider [-h] [-p] [-s  [...]] [-i  [...]] [-t] [-init] [-j] [-sj]
    ...
    optional arguments:
      ...
      -p , --project        创建项目 如 fastspider create -p <project_name>
      -s  [ ...], --spider  [ ...]
                            创建爬虫 如 fastspider create -s <spider_name> <spider_type>
                            spider_type=1 AirSpider; spider_type=2 Spider;
                            spider_type=3 BatchSpider;
      -i  [ ...], --item  [ ...]
                            创建item 如 fastspider create -i test 则生成test表对应的item。
                            支持like语法模糊匹配所要生产的表。 若想生成支持字典方式赋值的item，则create -item
                            test 1
      -t , --table          根据json创建表 如 fastspider create -t <table_name>
      -init                 创建__init__.py 如 fastspider create -init
      -j, --json            创建json
      -sj, --sort_json      创建有序json
      --setting             创建全局配置文件 fastspider create -setting
      --host                mysql 连接地址
      --port                mysql 端口
      --username            mysql 用户名
      --password            mysql 密码
...
命令
    ...
    fastspider create -p <project_name>

示例：

    fastspider create -p first-project
...
命令
    ...
    fastspider create -s <spider_name> <spider_type>
...
AirSpider爬虫示例：
    ...
    fastspider create -s first_spider 1
...
生成first_spider.py, 内容如下：
    ...
    class FirstSpider(fastspider.AirSpider):
        def start_requests(self):
            yield fastspider.Request("https://www.baidu.com")
...
3. 进入items目录，执行命令
        ...
        fastspider create -i spider_data
...
class SpiderDataItem(Item):
    """
    This class was generated by fastspider.
    command: fastspider create -i spider_data.
    """
...
若不配置setting.py, 可在命令行中指定数据库连接信息
     ...
     fastspider create -i spider_data --host localhost --db fastspider --username fastspider --password fastspider123
...
**若item字段过多，不想逐一赋值，可通过如下方式创建**
    ...
    fastspider create -i spider_data 1
...
class SpiderDataItem(Item):
    """
    This class was generated by fastspider.
    command: fastspider create -i spider_data 1.
    """
...
命令：
    ...
    fastspider create -t <table_name>

用的示例：

    > fastspider create -t test
...
下载调试器，可方便测试抽取规则是否正确，输入`fastspider shell -h`查看使用帮助

    > fastspider shell -h

    下载调试器

    usage: fastspider shell [options] [args]
...
1. 以url为例，请求百度：
    ...
    请求
        ...
        > fastspider shell -u https://www.baidu.com
...
1. 以curl为例，请求百度（通常用来测试post接口比较方便）
    ...
    1. 测试 输入`fastspider shell --`, 粘贴刚刚复制的curl

            > fastspider shell --curl 'https://www.baidu.com/' \
