...
创建爬虫
    ...
    fastspider create -s spider_test
...
1. 下发任务：下发1~14页列表任务
        ...
        def start_requests(self):
            for i in range(1, 15):
                yield fastspider.Request("https://www.qiushibaike.com/8hr/page/{}/".format(i))
...
派发详情任务：
    ...
    yield fastspider.Request(url, callback=self.parser_detail)  # callback 为回调函数

若我们需要携带title字段，写法如下

    yield fastspider.Request(url, callback=self.parser_detail, title=title)  # title 为携带的字段，直接携带就行，不用放到一个meta里
...
## 整体代码如下
    ...
    class SpiderTest(fastspider.AirSpider):
        def start_requests(self):
            for i in range(1, 15):
                yield fastspider.Request("https://www.qiushibaike.com/8hr/page/{}/".format(i))
